...
\textbf{Drugi zbiór danych} -- \textit{License Plate Text Recognition Dataset}
(N.~Yazdani) -- zawiera tysiące prawdziwych cropów tablic z etykietami
\textbf{tekstowymi} (bez bounding boxów znaków). Wykorzystany do
pseudo-etykietowania (sekcja \emph{Opis metod}).

\vspace{0.4cm}

W celu zmierzenia \textbf{jakości odczytywania numerów} posłużyliśmy się
...
zawiera tysiące prawdziwych cropów tablic z etykietami
\textbf{tekstowymi} (bez bounding boxów znaków). Wykorzystany do
pseudo-etykietowania.
...
\begin{frame}{Generator syntetycznych tablic}
    ...
    Wygenerowano \textbf{12 000} obrazów (\texttt{.jpg} + etykiety YOLO),
    w tym \textbf{$\sim$30\% tablic dwurzędowych}.

    \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{pics/syntatyki2.png}
        \vspace{0.1cm}
        {\scriptsize Porównanie syntetyków z prawdziwymi cropami}
    \end{figure}

    Syntetyki trafiają \textbf{tylko do zbioru treningowego} -- val/test pozostają
    w 100\% prawdziwe.
...
Drugie źródło prawdziwych danych treningowych -- \textbf{za darmo}.

\begin{itemize}
    \item Drugi dataset ma tysiące prawdziwych cropów z etykietami \textbf{tekstowymi},
    ...
    (syntetyki + 146 prawdziwych + pseudo-etykietowane prawdziwe).
\end{itemize}
